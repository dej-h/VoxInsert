...
constexpr char kMistralTranscriptionUrl[] = "https://api.mistral.ai/v1/audio/transcriptions";
...
constexpr char kMistralOfflineTranscriptionModel[] = "voxtral-mini-latest";
...
std::wstring BuildHttpFailureReason(const cpr::Response& response) {
    ...
    try {
        ...
        const auto detailIt = body.find("detail");
        if (detailIt != body.end() && detailIt->is_string()) {
            bodyMessage = detailIt->get<std::string>();
        }

        const auto errorIt = body.find("error");
        if (bodyMessage.empty() && errorIt != body.end()) {
            if (errorIt->is_string()) {
                bodyMessage = errorIt->get<std::string>();
            }
            else if (errorIt->is_object()) {
...
std::string_view MistralTranscriptionService::ProviderId() const noexcept {
    return "mistral";
}

bool MistralTranscriptionService::Transcribe(
    const TranscriptionConfig& config,
    const std::filesystem::path& wavPath,
    std::string& transcript,
    std::wstring& failureReason) const {
    if (config.mistral.model != kMistralOfflineTranscriptionModel) {
        failureReason = L"Mistral offline transcription currently supports model '";
        failureReason += WideFromUtf8(kMistralOfflineTranscriptionModel);
        failureReason += L"'. The configured model '";
        failureReason += WideFromUtf8(config.mistral.model);
        failureReason += L"' is not valid for /v1/audio/transcriptions. Use Voxtral Small only on the chat completions endpoint with audio input.";
        return false;
    }
    ...
    if (!TryReadApiCredentialSecret(CredentialTargetFromConfig(config), apiKeyWide, failureReason)) {
        return false;
    }
    ...
    const std::string normalizedContextBias = NormalizeMistralContextBias(config.mistral.contextBias);

    std::wstring lastFailureReason;
    for (int attempt = 1; attempt <= kMaxTranscriptionAttempts; ++attempt) {
        const cpr::Multipart multipart = normalizedContextBias.empty()
            ? cpr::Multipart{
            ...
            : cpr::Multipart{
                {"model", config.mistral.model},
                {"language", config.languageHint},
                {"context_bias", normalizedContextBias},
                {"file", wavBuffer, "audio/wav"}
            };

        const cpr::Response response = cpr::Post(
            cpr::Url{kMistralTranscriptionUrl},
